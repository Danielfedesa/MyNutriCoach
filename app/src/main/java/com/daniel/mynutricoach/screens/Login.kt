    ...
    Column(modifier = Modifier
        ...
        Button(onClick = {
            ...
            if (email.isEmpty() || password.isEmpty()) {
                ...
                return@Button // Salimos del bloque de código del botón (Como un break)
            }

            // Si los campos no están vacíos, intentamos iniciar sesión
            auth.signInWithEmailAndPassword(email, password)
                .addOnCompleteListener { task ->

                    // Si el inicio de sesión es correcto, navegamos a la pantalla Home
                    if (task.isSuccessful) {
                        navController.navigate("Home")
                        Log.i("Login", "Inicio de sesión correcto")
                    } else {
                        // Si el inicio de sesión falla, mostramos un mensaje
